...
///map ne supporte pas plusieurs demande, en posant une question on passe ooutre.///
...
const inputText = document.getElementById("champ");
...
const fetchMovie = async ()=>{
    ...
    results = await fetch(ApiUri).then((response) => response.json())
    console.log(results.results)
} 

const moviesDiplay = async ()=>{
    await fetchMovie()
    exercice.innerHTML = results.results.map(film=>`
        <div><h2>${film.original_title}</h2>${(film.vote_average>=5)?('<i class="fa-regular fa-thumbs-up"></i>'):('<i class="fa-regular fa-thumbs-down"></i>')}
        <h2>${film.vote_average}/10</h2>
        <img ${(film.poster_path==undefined)?(`src="undefined.jpg"`):(`src="https://image.tmdb.org/t/p/w200${film.poster_path}"`)}>
        <p>V.O:${film.original_language}, sortie en ${film.release_date}<br>${film.overview}</p></div>
    `).join("")
}

form.addEventListener("submit", e=>{
    e.preventDefault();
    movie = inputText.value;
    moviesDiplay()
})
...
///////////////////////fetch("url").then((param)=>codeparam)